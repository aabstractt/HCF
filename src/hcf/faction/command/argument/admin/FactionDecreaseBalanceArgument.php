...
class FactionDecreaseBalanceArgument extends Argument {
    ...
    public function run(CommandSender $sender, string $commandLabel, string $argumentLabel, array $args): void {
        ...
        if (count($args) < 2) {
            $sender->sendMessage(TextFormat::RED . 'Usage: /' . $commandLabel . ' decreasebalance <faction> <value>');
        ...
        $faction->decreaseBalance($value);

        $sender->sendMessage(Placeholders::replacePlaceholders('FACTION_BALANCE_DECREASED', $faction->getName(), (string) $value));
    }
}